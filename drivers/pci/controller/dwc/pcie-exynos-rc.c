...
#endif /* CONFIG_GS_S2MPU || CONFIG_EXYNOS_PCIE_IOMMU */
				       ...
				       struct exynos_pcie *exynos_pcie)
{
	...
	/*
	 * Hand the elbi mapping to the dwc framework so it skips its own
	 * devm_ioremap_resource() call (would otherwise -EBUSY on the same
	 * region). Done after we already have it ioremap'd.
	 */
	exynos_pcie->pci->elbi_base = exynos_pcie->elbi_base;
...
skip_sep_request_irq:
	...
	/*
	 * pp->msi_domain was removed from dw_pcie_rp in mainline; this block
	 * patched the bottom-chip ops for affinity/wake/mask. Skipped for now
	 * — bringup-only. Revisit when needed.
	 */
